...
构建时分别定义 `LUAPROF_EXPECT_LUA_VERSION=504` 和 `505`。不能互换两个 module，
也不能把另一份 `liblua.a` 静态链接进 module。
...
PUC Lua bridge 的完整修改范围见[项目接入指南](integration.md#32-移植到项目自己的-lua)。
bridge 改动通常需要同时更新：
...
不要只修改父项目的 Lua fork。若 contract 也适用于 Skynet，必须将等效改动移植到
...
- 两个 gitlink commit 已经推送到各自 `luaprof` 分支。
- 中英文 README 和 docs 入口有效。
- 两个 module 使用正确 Lua ABI，且没有静态包含另一份 Lua。
- `make test` 和 `make test-skynet` 通过。
- thread-per-VM 与 Skynet 示例能生成可由标准 `go tool pprof` 读取的结果。
- 父仓库和两个 fork 没有维护者遗留的未提交修改。
